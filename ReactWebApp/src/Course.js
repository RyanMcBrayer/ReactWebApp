...
class Course extends React.Component {
  ...
  getCourseButton() {
    ...
    return (
      <Button className="me-1" variant={buttonVariant} onClick={buttonOnClick}>
        {buttonText}
      </Button>
    );
  }
  ...
  getExpansionButton() {
    ...
    let buttonText = this.state.expanded ?  "Less Info" :  "More Info";
    let buttonOnClick = () => this.setExpanded(!this.state.expanded);
    let buttonStyle = this.state.expanded ? "outline-primary" : "primary";
    return (
      <Button
        variant={buttonStyle}
    ...
    return (
      <Card className="col-6 mb-2 p-2">
        ...
        {!this.props.ratingMode && (
          <Button className="mt-2" variant="primary" onClick={() => this.openModal()}>
